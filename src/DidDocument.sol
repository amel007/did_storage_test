...
contract DidDocument is IDidDocument {
    ...
    function init(address issuerAddr) public {
        require(msg.pubkey() == _didItem.issuerPubKey);
    ...
    function newDidIssuerPubKey(uint256 pubKey, address issuerAddr) public override onlyOwner() {
        tvm.rawReserve(Constants.MIN_FOR_CONTRACT, 2);
        _didItem.issuerPubKey = pubKey;
        _didItem.issuerAddr = issuerAddr;
        msg.sender.transfer({value: 0, flag: 128});
    }
